...
## A while loop to check if the exit code is equal to 0, meaning the username already extists
while [ $? -eq 0 ]
do
...
## While loop to check if the exit code is equal to 0, which means the group already exists
while [ $? -eq ]
do
echo -n "[-] THe group already exista."
echo ""
...
# Once a unique group name is chosen, we proceess to create it
groupadd $group 
